...
y_test <- read.table("y_test.txt") #Read 'y_test' file
        y_test2 <- join(y_test, activity_labels, by="V1")[2] #Converts numbers to labels
        colnames(y_test2) <- "Activity" ###Step 3 (2 of 2) Complete

test_table <- cbind(subject_test, y_test2, x_test) #Combine all TEST data into one table

full_table <- rbind(train_table,test_table) ###Step 1 Complete

mean_std <- grep("mean\\(\\)|std\\(\\)", colnames(full_table)) #Stores all columns with the strings "mean()" or "std()"
full_table_MeanStd <- full_table[c(1,2,mean_std)] ###Step 2 Complete
...
var_names <- gsub("-","_", colnames(full_table_MeanStd), fixed = TRUE) #Replace all '-' with '_' in all columns
	colnames(full_table_MeanStd) <- var_names ###Step 4 Complete

TidyData <- full_table_MeanStd %>%
	group_by(Subject, Activity) %>%
	summarize_all(funs(mean)) ###Step 5 Complete. Tidy data set with average of each variable for each Activity and each Subject.
